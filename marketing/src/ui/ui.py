...
def show_settings(
    ...
    with right_of_title_space:
        ...
        start_date = src_data.encounters_df.encounter_date.min()
        end_date = src_data.encounters_df.encounter_date.max()
        ...
        with cols[0]:
            selected_month = st.selectbox(
                ...
                options=months,
                index=(
                    list(months).index(default_month) if default_month in months else 0
                ),
...
def show_content(settings: settings.Settings, data: app_data.AppData):
    ...
    x_min = data.first_date.replace(day=1)
    x_max = data.last_date + pd.offsets.MonthEnd(0)
    ...
    for clinic in data.clinics:
        ...
        month_end = selected_month + pd.offsets.MonthEnd(0)
        clinic_data = data.encounters_df[
            (data.encounters_df["clinic"] == clinic)
            & (data.encounters_df["encounter_date"] >= month_start)
            & (data.encounters_df["encounter_date"] <= month_end)
        ]
        ...
        with cols[0]:
            ...
            patient_first_visits = (
                data.encounters_df[data.encounters_df["clinic"] == clinic]
                .groupby("prw_id")["encounter_date"]
                .min()
            )
            ...
            new_patients = len(
                clinic_data[
                    clinic_data["prw_id"].map(patient_first_visits) >= month_start
                ]
            )

            # Calculate percentage
            new_patient_pct = (
                (new_patients / total_visits * 100) if total_visits > 0 else 0
            )
            st_util.st_card(
        ...
        with cols[1]:
            ...
            no_show_data = data.no_shows_df[
                ...
                & (data.no_shows_df["encounter_date"] >= month_start)
                & (data.no_shows_df["encounter_date"] <= month_end)
            ]
        ...
        with st.expander("By Month", expanded=True):
            ...
            clinic_no_shows = data.no_shows_df[
                data.no_shows_df["clinic"] == clinic
            ].copy()
            ...
            monthly_volumes["no_show_pct"] = (
                monthly_volumes["no_shows"] / monthly_volumes["total_visits"] * 100
            ).fillna(0)
            monthly_volumes["new_patient_pct"] = (
                monthly_volumes["new_patients"] / monthly_volumes["total_visits"] * 100
            ).fillna(0)
