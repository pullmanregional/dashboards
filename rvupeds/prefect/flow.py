...
PRH_RVUPEDS_VENV_NAME = os.environ.get("PRH_RVUPEDS_VENV_NAME", "")
PRH_RVUPEDS_CLOUDFLARE_R2_BUCKET = os.environ.get("PRH_RVUPEDS_CLOUDFLARE_R2_BUCKET")
PRH_RVUPEDS_DATA_KEY = (
    os.environ.get("PRH_RVUPEDS_DATA_KEY") or Secret.load("prh-rvupeds-data-key").get()
)
PRH_RVUPEDS_ENCRYPTED_DB_FILE = os.environ.get("PRH_RVUPEDS_ENCRYPTED_DB_FILE")
...
def prw_datamart_rvupeds():
    ...
    with ShellOperation(
        commands=[
            "pipenv install",
            f'pipenv run python ingest_datamart.py --prw "{PRW_CONN}" --out "{PRH_RVUPEDS_ENCRYPTED_DB_FILE}" --key "{PRH_RVUPEDS_DATA_KEY}"',
        ],
        env={
            "PIPENV_IGNORE_VIRTUALENVS": "1",
            "PIPENV_CUSTOM_VENV_NAME": PRH_RVUPEDS_VENV_NAME,
        },
        stream_output=True,
    ) as op:
        proc = op.trigger()
    ...
    if PRH_RVUPEDS_CLOUDFLARE_R2_BUCKET:
        upload_files(
            PRH_RVUPEDS_CLOUDFLARE_R2_BUCKET,
            [PRH_RVUPEDS_ENCRYPTED_DB_FILE],
        )
